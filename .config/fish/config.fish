...
set -gx REQUESTS_CA_BUNDLE /etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem
set -gx SSL_CERT_FILE /etc/pki/ca-trust/extracted/pem/tls-ca-bundle.pem


# english!
begin
    set -l english_commands git man nano

    for command in $english_commands
        alias $command="LC_ALL=en_US.utf8 command $command"
    end
end
...
if status is-interactive
    ...
    abbr -a sudo 'sudo -E'
    abbr -a sudop 'sudo -E env "PATH=$PATH"'
    ...
    abbr -a catl 'bat -pP'
    abbr -a batl 'bat -pP'
    abbr -a psgrep 'ps -ef | grep -nE'

    abbr -a copy 'fish_clipboard_copy'
    abbr -a se 'source.env'
    abbr -a --position anywhere ta 'tmux attach'
    abbr -a r 'ranger'
    abbr -a l 'lazydocker'
    abbr -a ts 'tailscale'
    ...
    abbr -a gshrb --set-cursor 'git stash && git rebase % && git stash pop'
    ...
    abbr -a --position anywhere g 'graftcp'
    abbr -a --position anywhere gl 'graftcp-local'
    abbr -a --position anywhere pc 'proxychains -q -f ~/.config/proxychains/'
    abbr -a --position anywhere pch 'proxychains -q -f ~/.config/proxychains/hiddify.conf'
    abbr -a sb-tun 'sudo -E env "PATH=$PATH" sb-tun ~/.sing-box-template.json'
    abbr -a sb-up 'sudo -E env "PATH=$PATH" sb-up ~/.sing-box-template.json'
    ...
    abbr -a mfo 'OLLAMA_HOST=$mflocal:11434 ollama'
    abbr -a codex 'graftcp codex'
    ...
    if test -f ~/.gitconfig
        set -l inside_alias_section false

        while read -l untrimmed_line
            set -l line (string trim $untrimmed_line)

            if test $line = "[alias]"
                set inside_alias_section true
                continue
            else if string match -q "[*]" $line  # any other section
                set inside_alias_section false
                continue
            end

            if $inside_alias_section; and test -n $line
                set -l alias_parts (string split -m 1 = $line | string trim)
                set -l alias_name $alias_parts[1]
                set -l alias_command $alias_parts[2]

                if test (string sub -l 1 $alias_command) != "!"
                    complete -c git -n "__fish_seen_subcommand_from $alias_name" --wraps "git $alias_cmd"
                end
            end
        end < ~/.gitconfig
    end
