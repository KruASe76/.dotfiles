...
//#region src/ui.ts
var PopupSwitchWithButtonMenuItem = class extends PopupMenu.PopupSwitchMenuItem {
	_client;
	_logger;
	_device;
	_showRefreshButton;
	...
	sync(device) {
		...
		if (device.connected) {
			if (device.batteryPercentage && device.batteryPercentage > 0) {
				...
				this._logger.log(`Battery percentage ${device.alias || device.name}: ${device.batteryPercentage}`);
	...
	_buildPendingLabel() {
		const label = new St.Label({ text: gettext("Wait") });
		label.hide();
		return label;
	}
	...
	activate(event) {
		...
		if (event.type() === Clutter.EventType.KEY_PRESS && event.get_key_symbol() === Clutter.KEY_space) return;
...
var BatteryInfoWidgetClass = class extends St.BoxLayout {
	_icon;
	_label;
	...
	setPercentage(value) {
		if (value == null) {
			...
			const iconName = `battery-level-${10 * Math.floor(value / 10)}-symbolic`;
...
//#endregion
export { PopupBluetoothDeviceMenuItem, PopupSwitchWithButtonMenuItem };