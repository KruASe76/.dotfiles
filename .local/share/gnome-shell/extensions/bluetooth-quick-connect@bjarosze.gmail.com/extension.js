...
var BluetoothQuickConnect = class extends Extension {
	_settings;
	_logger;
	_controller;
	_menu;
	...
	_connectControllerSignals() {
		...
		this._connectSignal(this._controller, "device-inserted", (_ctrl, device) => {
			...
			if (device.paired) this._addMenuItem(device);
			else this._logger.log(`Device ${device.alias || device.name} not paired, ignoring`);
		});
		this._connectSignal(this._controller, "device-changed", (_ctrl, device) => {
			...
			else this._logger.log(`Skipping change event for unpaired device ${device.alias || device.name}`);
		});
	...
	_syncMenuItem(device) {
		...
		(this._items[device.address || ""] || this._addMenuItem(device)).sync(device);
	}
	_addMenuItem(device) {
		this._logger.log(`Adding device menu item: ${device.alias || device.name} ${device.address}`);
		const menuItem = new PopupBluetoothDeviceMenuItem(this._controller._client, device, this._logger, {
	...
	_connectIdleMonitor() {
		...
		this._idleMonitorId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, this._settings.autoPowerOffCheckingInterval() * 1e3, () => {
			if (this._settings.isAutoPowerOffEnabled() && this._controller.getConnectedDevices().length === 0) this._proxy.BluetoothAirplaneMode = true;
			return true;
		});
	}
	...
	_disconnectSignals() {
		...
		for (const signal of this._signals) signal.subject.disconnect(signal.signal_id);
		this._signals = [];
	}
	...
	_addDevicesToMenu(skipDevice = null) {
		...
		})) if (device.paired && device.get_object_path() !== skipDevice) this._addMenuItem(device);
		else this._logger.log(`skipping adding device ${device.alias || device.name}`);
	}
	_removeDevicesFromMenu() {
		for (const item of Object.values(this._items)) item.destroy();
		this._items = {};
	}
};

//#endregion
export { BluetoothQuickConnect as default };